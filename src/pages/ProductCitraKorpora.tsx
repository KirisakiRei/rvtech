...
import { Building2, Users, Briefcase, MapPin, Globe, FileText } from 'lucide-react'
...
const features = [
  { icon: Building2, title: 'Company Profile Lengkap', description: 'Halaman about, visi-misi, sejarah perusahaan — semua tersaji dengan desain profesional yang meyakinkan.' },
  { icon: Users, title: 'Halaman Tim', description: 'Tampilkan anggota tim dengan foto, jabatan, dan bio singkat. Beri wajah di balik brand-mu.' },
  { icon: Briefcase, title: 'Portofolio & Services', description: 'Showcase proyek terbaik dan layanan yang ditawarkan. Buat calon klien terkesan sejak pandangan pertama.' },
  { icon: MapPin, title: 'Kontak & Lokasi', description: 'Formulir kontak, embed Google Maps, dan semua informasi yang dibutuhkan klien untuk menjangkau.' },
  { icon: Globe, title: 'Custom Domain', description: 'Gunakan domain sendiri seperti namaPerusahaan.com untuk kesan yang lebih profesional dan terpercaya.' },
  { icon: FileText, title: 'Multi-Page Website', description: 'Bukan cuma satu halaman — buat beberapa halaman untuk layanan, blog, dan informasi detail lainnya.' },
]

export function ProductCitraKorpora() {
  const product = PRODUCTS.find((p) => p.id === 'citrakorpora')!
  const pricing = PRODUCT_PRICING.citrakorpora
    ...
    <>
      ...
      <main>
            ...
            <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-center min-h-[60vh]">
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <Link to="/daftar?product=citrakorpora&tier=pro" id="citrakorpora-hero-cta" className="w-full sm:w-auto">
              ...
              <div className="lg:col-span-7 relative h-full min-h-[400px] hidden lg:block">
                ...
                >
                  <div className="w-12 h-12 rounded-full flex items-center justify-center shrink-0 border" style={{ backgroundColor: `${product.color}15`, borderColor: `${product.color}30` }}>
                    <Building2 className="w-5 h-5 fill-current/20" style={{ color: product.color }} />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-foreground leading-tight">Identitas Profesional</p>
                    <p className="text-[11px] text-muted-foreground mt-0.5">Tampil Meyakinkan</p>
                  </div>
                </motion.div>
                ...
                >
                  <div className="w-12 h-12 rounded-full bg-success/10 flex items-center justify-center shrink-0 border border-success/20">
                    <Globe className="w-5 h-5 text-success fill-success/20" />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-foreground leading-tight">Go Global</p>
                    <p className="text-[11px] text-muted-foreground mt-0.5">Jangkau Klien Lebih Luas</p>
                  </div>
                </motion.div>
              </div>
            </div>
        ...
        <ProductScrollFeatures 
          features={features}
          color={product.color}
          title="Kesan profesional yang tak terlupakan"
          subtitle="Fitur Unggulan"
        />

        <ProductPricingSection productId="citrakorpora" basic={pricing.basic} pro={pricing.pro} />
      </main>
      <Footer />
    </>
  )
